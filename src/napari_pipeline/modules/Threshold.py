...
""",
    ...
    def threshold_operation(self):
        if self.threshold_scope == TS_GLOBAL:
            return self.global_operation

        return self.local_operation
    ...
    def run(self, input_image, workspace):
        ...
        return output
        # workspace.image_set.add(self.y_name, output)
    ...
    def get_threshold(self, input_image, workspace, automatic=False):
        ...
        if self.threshold_operation == TM_MANUAL:
            final_threshold = self.manual_threshold 
            orig_threshold = self.manual_threshold
            guide_threshold = None 
            binary_image, sigma = _apply_threshold(
            ...
            final_threshold *= self.threshold_correction_factor
            final_threshold = min(max(final_threshold, self.threshold_range[0]), self.threshold_range[1])
            guide_threshold = None 
            binary_image, sigma = _apply_threshold(
                input_image.pixel_data,
                threshold=final_threshold,
            ...
            if self.threshold_scope == "Global":
                if self.global_operation == "Otsu" and self.two_class_otsu == "Three classes":
                    ...
                    threshold_method = self.convert_setting(self.global_operation)
            elif self.threshold_scope == "Adaptive":
                if self.local_operation == "Otsu" and self.two_class_otsu == "Three classes":
                    threshold_method = "multiotsu"
                else:
                    threshold_method = self.convert_setting(self.local_operation)
            final_threshold, orig_threshold, guide_threshold, binary_image, sigma = _threshold(
                    input_image.pixel_data,
                    mask=input_image.mask,
                    threshold_scope=self.threshold_scope,
                    threshold_method=threshold_method,
                    assign_middle_to_foreground=self.assign_middle_to_foreground,
                    log_transform=self.log_transform,
                    threshold_correction_factor=self.threshold_correction_factor,
                    threshold_min=self.threshold_range[0],
                    threshold_max=self.threshold_range[1],
                    ...
                    variance_method=self.convert_setting(self.variance_method),
    ...
    #         title="Original image: {}".format(self.x_name),
    #     )
    ...
    #         title="Thresholded image: {}".format(self.y_name),
    ...
    def validate_module(self, pipeline):
        if (
            ...
            and self.lower_outlier_fraction + self.upper_outlier_fraction
            >= 1
        ):
            raise ValidationError(
                ...
                less than one.
                """.format(
                    self.lower_outlier_fraction, self.upper_outlier_fraction
                ),
                self.upper_outlier_fraction,
            )
    ...
    **kwargs,
    ...
    if volumetric:
        ...
        for z in range(image.shape[0]):
            thresh_out[z, :, :] = _get_adaptive_threshold(
                image[z, :, :],
    ...
    global_threshold = _get_global_threshold(
        image,
        mask,
        threshold_method,
        threshold_min,
...
def _apply_threshold(image, threshold, mask=None, smoothing=0):
    if mask is None:
...
def _threshold(
    ...
    **kwargs,
    ...
    if threshold_scope.casefold() == "adaptive":
        final_threshold = _get_adaptive_threshold(
            image,
            mask=mask,
            threshold_method=threshold_method,
            window_size=window_size,
        ...
        orig_threshold = _get_adaptive_threshold(
            image,
            mask=mask,
            threshold_method=threshold_method,
            window_size=window_size,
        ...
        final_threshold = _get_global_threshold(
            image,
            mask=mask,
            threshold_method=threshold_method,
            threshold_min=threshold_min,
